...
/*   Updated: 2024/05/29 16:28:20 by dnovak           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

// 1.	Change isalnum to name of your function.
// 2.	In ft_putfncinput change number of parameters for your function
// 3.	Add your tests, don't forget to add them in ft_isalnum_test
...
static int	test1(int index, int full, char *name, int c)
{
	...
	// 2. TEST
	int check = 0;
	int users = ft_isalnum(c);
	int expect = isalnum(c);
	...
	if (check == 0 || full == 1)
	{
		...
		printf("\tTest %i: ", index);
		ft_putfncinput(name, (char) c); //NEED to change this function
		printf(" -> ");
