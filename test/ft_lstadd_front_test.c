...
/*   Updated: 2024/05/31 14:56:16 by dnovak           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

// 1.	Change lstadd_front to name of your function.
// 2.	In ft_putfncinput change number of parameters for your function
// 3.	Add your tests, don't forget to add them in ft_lstadd_front_test
...
static int	test1(int index, int full, char *name, t_list **lst, t_list *new, t_list *expect)
{
	// 1. Setting up input
	...
	if (check == 0 || full == 1)
	{
		...
		printf(" -> ");
		printf("users: %p, expected: %p\n", ft_lstlast(*lst), expect); //NEED to change this print
	}
...
{
	char	*name = "ft_lstadd_front_test";
	int		check = 1;
	int		index = 1;
	...
	first->next = second;
	second->next = last;
	check *= test1(index++, full, name, &lst, new, last);
	check *= test1(index++, full, name, &null_node, new, new);
	lst = first;
	check *= test2(index++, full, name, &lst, NULL, last);
	free(first);
	free(second);
	free(last);
	free(new);
